...
function renderTable(area){
    ...
    DATA.forEach((r,i)=>{
        ...
        const rowStyle=hasPendingTx?'background:#fffbeb':isExp?'background:#fff5f5':isLow&&mine?'background:#fffbeb':'';
        ...
        h+=`<tr class="${mine?'me':''}${hasPendingTx?' pending-transfer':''}" style="${rowStyle}" onclick="openDetail(${r.id})">
...
function renderGrid(area){
    ...
    DATA.forEach(r=>{
        ...
        h+=`<div class="stk-card${mine?' me':''}${SELECTED.has(+r.id)?' stk-selected':''}${!!r.pending_transfer_id?' pending-transfer':''}" onclick="openDetail(${r.id})" style="padding-bottom:${CAN_EDIT?'0':'0'}">
...
function renderCompact(area){
    ...
    DATA.forEach(r=>{
        const p=parseFloat(r.remaining_percentage)||0;
        const mine=r.is_mine;
        h+=`<div class="stk-cr${mine?' me':''}${SELECTED.has(+r.id)?' stk-selected':''}${!!r.pending_transfer_id?' pending-transfer':''}" onclick="openDetail(${r.id})">
...
/* ═════════════════════════════════════════
   TRANSACTION MODAL (เบิก / ยืม / โอน)
   ═════════════════════════════════════════ */
let _txnType='use',_txnUser=null,_txnSearchTimer=null;
...
function buildTxnForm(type,c,maxQty,unit){
    ...
    const chemInfo=`<div class="txn-chem-info">
        <div class="txn-chem-ic"><i class="fas fa-flask"></i></div>
        <div class="txn-chem-body">
            <div class="txn-chem-name">${esc(c.chemical_name||'-')}</div>
            ${c.bottle_code?`<div class="txn-chem-code"># ${esc(c.bottle_code)}</div>`:''}
            <div class="txn-chem-stats">
                <div class="txn-chem-qty">
                    ...
                    <div class="txn-chem-qty-lbl">${T('คงเหลือ','remaining')}</div>
                </div>
                ${pct!==null?`
                <div class="txn-chem-divider"></div>
                <div class="txn-chem-meta">
                    <span><i class="fas fa-chart-bar"></i> ${pct}% ${T('ของปริมาณเริ่มต้น','of initial')}</span>
                    ${c.location_name?`<span><i class="fas fa-map-marker-alt"></i> ${esc(c.location_name)}</span>`:''}
                </div>`:''}
            </div>
        </div>
    </div>`;
    ...
    const qtyRow=`<div class="txn-field">
        ...
        <div class="txn-qty-row">
            <input type="number" id="txnQty" min="0.001" max="${maxQty}" step="any" placeholder="0" style="text-align:right">
    ...
    const userSearch=`<div class="txn-field" id="txnUserField">
        <label>${T('ผู้รับ / ผู้ยืม','Recipient / Borrower')} <span style="color:#dc2626">*</span></label>
        <div class="txn-user-wrap">
            <input type="text" id="txnUserSearch" autocomplete="off" placeholder="${T('พิมพ์ชื่อหรือ username ค้นหา...','Search by name or username...')}">
            <div class="txn-user-dropdown" id="txnUserDropdown" style="display:none"></div>
        </div>
        <div id="txnUserSelected" style="display:none"></div>
    </div>`;
    ...
    if(type==='use'){
        ...
        const isMine=c.is_mine||IS_ADMIN;
        const approvalNote=isMine?'':T('<b>หมายเหตุ:</b> การยืมจะต้องรอการอนุมัติจากเจ้าของ/ผู้จัดการ','<b>Note:</b> Borrow request will need approval from the owner/manager.');
        if(approvalNote) html+=`<div class="txn-warn"><i class="fas fa-clock"></i><span>${approvalNote}</span></div>`;
        html+=qtyRow+userSearch+returnDate+purposeRow;
        ...
        html+=qtyRow+userSearch+purposeRow;
    }
...
function setupTxnUserSearch(){
    const inp=document.getElementById('txnUserSearch');
    if(!inp) return;
    inp.addEventListener('input',function(){
        clearTimeout(_txnSearchTimer);
        const q=this.value.trim();
        if(q.length<2){document.getElementById('txnUserDropdown').style.display='none';return;}
        _txnSearchTimer=setTimeout(()=>doTxnUserSearch(q),300);
    });
    inp.addEventListener('blur',()=>{setTimeout(()=>{const dd=document.getElementById('txnUserDropdown');if(dd)dd.style.display='none';},200);});
}

async function doTxnUserSearch(q){
    const dd=document.getElementById('txnUserDropdown');
    if(!dd) return;
    dd.style.display='block';
    dd.innerHTML='<div class="txn-searching"><i class="fas fa-spinner fa-spin"></i> '+T('กำลังค้นหา...','Searching...')+'</div>';
    try{
        const res=await apiFetch('/v1/api/borrow.php?action=search_users&q='+encodeURIComponent(q));
        const users=Array.isArray(res)?res:(res.data||[]);
        if(!users.length){dd.innerHTML='<div class="txn-searching">'+T('ไม่พบผู้ใช้','No users found')+'</div>';return;}
        dd.innerHTML=users.map(u=>`
            <div class="txn-user-opt" onmousedown="selectTxnUser(${u.id},'${esc(u.full_name||u.username||'')}','${esc(u.department||'')}','${esc(u.username||'')}')">
                <div class="txn-user-av">${(u.full_name||u.username||'?').charAt(0).toUpperCase()}</div>
                <div>
                    <div class="txn-user-name">${esc(u.full_name||u.username||'')}</div>
                    <div class="txn-user-dep">${esc(u.department||u.username||'')}</div>
                </div>
            </div>`).join('');
    }catch(e){dd.innerHTML='<div class="txn-searching" style="color:#dc2626">'+T('เกิดข้อผิดพลาด','Error occurred')+'</div>';}
}
...
function selectTxnUser(id,name,dep,username){
    ...
    const inp=document.getElementById('txnUserSearch');
    const dd=document.getElementById('txnUserDropdown');
    const sel=document.getElementById('txnUserSelected');
    if(inp) inp.value='';
    if(dd) dd.style.display='none';
    if(sel){
        sel.style.display='flex';
        sel.className='txn-user-selected';
        sel.innerHTML=`<div class="txn-user-av" style="background:var(--accent)">${name.charAt(0).toUpperCase()}</div>
            <div><div class="txn-user-selected-name">${esc(name)}</div><div class="txn-user-selected-dep">${esc(dep||username||'')}</div></div>
            <button class="txn-user-clear" onclick="clearTxnUser()" title="${T('ล้าง','Clear')}"><i class="fas fa-times-circle"></i></button>`;
    }
}
...
async function submitTxn(type){
    ...
    // Determine source
    const srcType=c.source_type||'container';
    ...
    try{
        ...
        if(type==='borrow'){
            ...
            payload.expected_return_date=returnEl?returnEl.value:'';
        } else if(type==='transfer'){
            payload.to_user_id=_txnUser.id;
        }
